...
function CheckoutSection({ bodyDeviceId, cameraSize = "sm", ...props }: Props) {
  ...
  const checkOutInfo = useAppSelector((cardInfo) => {
    return props.position === LANE.LEFT
      ? cardInfo.checkOutCard.left
      : cardInfo.checkOutCard.right;
  });
  ...
  const {
    ...
    enabled:
      triggerGetInfoByPlateNumber.current && checkOutInfo.plateTextOut !== "",
  ...
  const {
    ...
    queryKey: [
      ...
      getValues("CardNumber")?.toString().trim(),
      timeOut,
    ],
    queryFn: () =>
      getCardCheckOutAPI(
        getValues("CardNumber")?.trim() ?? "",
        getLocalISOString(new Date(timeOut))
      ),
    enabled:
      triggerInfoByCard.current && getLocalISOString(new Date(timeOut)) !== "",
    refetchOnWindowFocus: false,
    refetchOnMount: false,
    retry: 0,
    gcTime: 0,
  ...
  const handleTriggerGetInfoByPlate = () => {
    ...
    dispatch(
      setNewCardInfo({
        ...
        info: {
          ...checkOutInfo,
          plateTextOut: unFormatPlateNumber(getValues("PlateNumber")),
        },
      })
    );
  ...
  const handlePlateDetection = useCallback(async () => {
    try {
      ...
      let plateRead = checkOutInfo.plateTextOut;
      let message = "";
      let croppedImage = "";
      ...
      //* separate parking with plate detection
      dispatch(
        setNewCardInfo({
          lane: props.position,
          info: {
  ...
  useEffect(() => {
    const plateIn = unFormatPlateNumber(checkOutInfo.plateTextIn);
    const plateOut = unFormatPlateNumber(checkOutInfo.plateTextOut);
    if (plateIn === "" && plateOut === "") {
      return;
    }
    let message = PLATE_MATCH;
    const isMatched = plateIn === plateOut;
    if (!isMatched) {
  ...
  }, [checkOutInfo.plateTextIn, checkOutInfo.plateTextOut]);
  ...
  useEffect(() => {
    const cardNumber = getValues("CardNumber")?.toString().trim();
    if (!cardNumber) {
      return () => {};
    }
    ...
    if (checkOutInfo.plateTextOut.trim() === "") {
      message = PLATE_NOT_READ;
      isError = true;
    }
    // Check if there's any actual change in state before dispatching
